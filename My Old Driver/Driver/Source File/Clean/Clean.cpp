...
bool Clean::CleanUnloadedDrivers()
{
	...
	pRtlProcessModules Modules = (pRtlProcessModules)ExAllocatePoolWithTag(NonPagedPool, Bytes, 0xFFFFFFFFFFFF);
	...
	PVOID NewBuffer = ExAllocatePoolWithTag(NonPagedPoolNx, 0x7D0, 0xFFFFFFFFFFFF);
	...
	*(uint64_t*)MmUnloadedDrivers = (uint64_t)NewBuffer;
	...
	ExFreePoolWithTag((PVOID)BufferPtr, 0xFFFFFFFFFFFF);

	DebugPrint("[+] Cleaned UnloadedDrivers \n");
